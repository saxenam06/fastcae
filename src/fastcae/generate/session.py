"""The rib work on one open project: its card, the spec written from it, and designs made from that.

What the routes hold between requests - and what the agent's tools work on, when there is an agent;
nothing here needs one. A card is filled, written as a new version of the spec when the engineer
asks for a design, and the design is made from the spec, never from the card: the spec is the
